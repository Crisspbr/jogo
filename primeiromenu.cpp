...
int menu(int lin1, int col1, int qtd, char lista[3][40]){
     ...
     while(1){
       ...
       tecla= getch();
       linhaCol(22,1);
       //printf(" tecla:  %d   ",tecla); //TESTE DO GETCHAR 
       //Opção
       if(tecla==27){ //ESC
       opc=0; break;
       }
       ...
       //Seta para cima
       else if(tecla==72){ //seleciona o item anterior - seta para cima
            if(opc>1)opc--;  // se opcao for maior que 1 volta
       ...
       //printf("tecla: %d ",opc); -> TESTE DAS TECLAS  
       }
     }
     return opc;
     }

int menu(int lin1, int col1, int qtd, char lista2[3][60]){
     int opc=1, lin2, col2, linha,i,tamMaxItem, tecla;
     
     //calcula as coordenadas
     tamMaxItem = strlen(lista2[0]);
     //tamanho maximo do item
     for(i=1; i<qtd;i++){
       if(strlen(lista2[i])>tamMaxItem){
          tamMaxItem = strlen(lista2[i]); 
       } 
     }
     lin2 = lin1+(qtd*2+2);
     col2 = col1+tamMaxItem+4;
...
{
    int opc;
    char lista[5][40]={"Jogar", "Como Jogar?", "Ranking", "Dificuldade", "Sair"};
